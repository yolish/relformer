...
class CameraPoseLoss(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.normalize_rot = config.get("rot_dim") == 4

    def forward(self, est_pose, gt_pose):
            """
            Forward pass
            :param est_pose: (torch.Tensor) batch of estimated poses, a NxK tensor
            :param gt_pose: (torch.Tensor) batch of ground_truth poses, a NxK tensor
            :return: camera pose loss
            """
            # Position loss
            l_x = torch.norm(gt_pose[:, 0:3] - est_pose[:, 0:3], dim=1, p=self.norm).mean()
            #  Orientation loss
            if self.normalize_rot:
                # normalized quaternion to unit norm
                l_q = torch.norm(F.normalize(gt_pose[:, 3:], p=2, dim=1) - F.normalize(est_pose[:, 3:], p=2, dim=1),
                                 dim=1, p=self.norm).mean()
            else:
                l_q = torch.norm(gt_pose[:, 3:] - est_pose[:, 3:], dim=1, p=self.norm).mean()
